      ...
      >
        ...
        >
          ...
          <el-form-item
            label="设置商品"
            prop="type"
            v-if="temp.type=='normal'"
          >
            <div v-show="selGoodsData[temp.type].goods_name">
              {{selGoodsData[temp.type].goods_name}}
              <el-link
                type="primary"
                :underline="false"
                @click="selGoodsData[temp.type]={};temp.goods_image=[];temp.goods_text=''"
              >删除</el-link>
            </div>
            <div>
              <el-button
                type="primary"
                @click="showSelGoodsPop"
              >
                选择商品
              </el-button>
            </div>
            <div class="cl-gray font12">
              设置商品后, 商品信息会覆盖当前商品图片及商品详情
            </div>
          </el-form-item>
          <el-form-item
            label="设置优惠券"
            prop="type"
            v-if="temp.type=='coupon'"
          >
            <div v-show="selGoodsData[temp.type].goods_name">
              {{selGoodsData[temp.type].coupon_name}}
              <el-link
                type="primary"
                :underline="false"
                @click="temp.coupon_id=''"
              >删除</el-link>
            </div>
            <div>
              <el-button
            ...
            <div>
              ...
              >
                <el-input
                  ...
                  type="number"
                >
                  <template slot="prepend">每人共</template>
              ...
              >
                <el-input
                  placeholder=""
                  v-model="temp.total_day"
                >
                  <template slot="prepend">每天提供</template>
                  <template slot="append">份</template>
                </el-input>
              </div>
            </div>
            <div class="cl-gray font12">每人可领限制 0表示不限制</div>

          </el-form-item>
          <el-form-item label="活动消耗">
            <div>
              <div
                class="i-b"
                style="width:300px"
              >
                <el-input
                  placeholder=""
                  v-model="temp.integral"
                  type="number"
                >
                  <template slot="prepend">消耗</template>
        ...
        >
          <el-checkbox
            v-model="f_isSpec"
            :disabled="temp.type=='coupon'"
          >启用商品规格</el-checkbox>
          <div
            class="cl-gray font12 mt"
            v-show="temp.type=='coupon'"
          >
            优惠券类型下不能编辑商品规格
          </div>
          <div
                  ...
                  >
                    ...
                    <td>
                      <el-input
                        size="small"
                        type="number"
                        v-model.number="childProductArray[index].integral"
                        placeholder="输入兑换所需积分"
                        style="width:120px;"
                      >
                      </el-input>
                    </td>
                    <td>
                      <el-input
                        size="small"
                        type="number"
                        v-model.number="childProductArray[index].money"
                        placeholder="输入兑换所需金额"
                        style="width:120px;"
                      >
                      </el-input>
                    </td>
                    <td>
                      <el-input
                        size="small"
                        type="number"
                        v-model.number="childProductArray[index].stock_num"
                        placeholder="输入库存数量"
                        style="width:120px;"
                      >
                      </el-input>
                  ...
                  <tr>
                    ...
                    >
                      ...
                      <template v-if="isSetListShow">
                        ...
                        >市场价</el-button>
                        <el-button
                          @click="openBatch('integral')"
                          size="mini"
                        >兑换所需积分</el-button>
                        <el-button
                          @click="openBatch('money')"
                          size="mini"
                        >兑换所需金额</el-button>
                        <el-button
                          @click="openBatch('stock_num')"
                          size="mini"
                        >库存数量</el-button>
...
type="number"
    ...
    <el-dialog
      :title="temp.type=='normal'? '选择商品':'选择优惠券'"
      :visible.sync="diaSelVisible"
    >
      <TableList
        v-if="diaSelVisible"
        @search="searGoods"
        :table-list="diaSelData.list"
        :tableMap="diaSelTableMap[temp.type]"
        :total="diaSelData.total"
        :slot-col="['goods_image','discounts_type']"
      >
...
<script>
  ...
  export default {
    ...
    watch: {
      'temp.type': function (newval, oldval) {
        if (newval == 'normal') {
          this.temp.is_spec = 1
    ...
    created() {
      ...
      if (this.$route.name === 'goods-add') {
        this.modiType = 'add'
      } else {
        this.modiType = 'edit'
        this.temp.goods_id = this.$route.query.goods_id
      ...
      // this.modiType = this.$route.query['operation']
      // this.editsData.goods_id = this.$route.query['goods_id']
      // this.basicEditType = this.$route.query['basicEditType']
      // if (this.basicEditType) {
      //   if (this.basicEditType === 'basicEdit') {
      //     this.tabShow = [true, false, true]
      //     this.activeName = 'first'
      //   }
      //   if (this.basicEditType === 'specEdit') {
    ...
    methods: {
      ...
      submit() {
        if (this.modiType === 'add') {
          api.integral_goods_create(this.modiTosendData()).then((res) => {
            this.$message({
              message: res.info,
              type: '',
              duration: 1300,
                onClose: () => {
                  if (res.code == 9999) {
                    this.$router.push('/credit-shop/list')
                  }
                }
            })
          })
        } else {
          api.integral_goods_edit(this.modiTosendData()).then((res) => {
            this.$message({
              message: res.info,
              type: '',
              duration: 1300,
      ...
      */
      ...
      // #region 基础信息
      selGoods(row) {
        console.log(row, 'selGoods');
        this.selGoodsData[this.temp.type] = row
        this.diaSelVisible = false

        if (this.temp.type == 'normal') {
          this.temp.goods_image = this.temp.goods_image.concat(this.selGoodsData[this.temp.type].goods_image)
          this.temp.goods_text = this.selGoodsData[this.temp.type].goods_text
          console.log(this.temp.goods_image.concat(this.selGoodsData[this.temp.type].goods_image), 'selGoods');
        } else if (this.temp.type == 'coupon') {
      ...
      showSelGoodsPop() {
        this.diaSelVisible = true
      },
      searGoods(query) {
        // todo 2020年5月13日 22:27:23
        if (this.temp.type == 'normal') {
          api.goods_share_list(query).then((res) => {
            this.diaSelData = res.data
          })
        } else if (this.temp.type == 'coupon') {
          api.coupon_list(query).then((res) => {
            this.diaSelData = res.data
          })
        }
      },
